...
public class Level1 extends GameState {
	...
	public void render(Graphics g) {
		...
		g.drawRect(frameX, frameY, grid(FRAME_WIDTH), grid(FRAME_HEIGHT));
	...
	public void tick() {
		if (!pause) {
			...
			// Just you should add tail objects after all that stuff, cuz otherwise this will mess it up
			if (tailReserve > 0) {
				addTail();
				tailReserve--;
			}
	...
	public void addFood(ID d) {
		// this should prevent food from appearing on the snake or on the other food
		...
		for (int g = 0; g < objects.size(); g++) {
			...
			if ((food.getX() != tempObj.getX() || food.getY() != tempObj.getY()) 
					&& (food.getX() + foodSize - 1 != tempObj.getX() || food.getY() + foodSize - 1 != tempObj.getY())) {
				...
				if (g == objects.size()-1) {
					...
					for (int j = 0; j < tail.size(); j++) {
						...
						if ((food.getX() != tempObjct.getX() || food.getY() != tempObjct.getY()) 
								&& (food.getX() + foodSize - 1 != tempObjct.getX() || food.getY() + foodSize - 1 != tempObjct.getY())) {
							continue;
						}
						else {
